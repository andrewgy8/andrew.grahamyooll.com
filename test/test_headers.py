...
class HeaderTest(unittest.TestCase):
    ...
    def test_single_header(self):
        text = "<!-- key1: val1 -->"
        headers = list(bakesite.read_headers(text))
    ...
    def test_headers_and_blank_line(self):
        text = "<!-- a: 1 -->\n<!-- b: 2 -->\n\n<!-- c: 3 -->\n"
        headers = list(bakesite.read_headers(text))
        self.assertEqual(headers, [("a", "1", 14), ("b", "2", 29), ("c", "3", 43)])

    def test_multiline_header(self):
        text = "<!--\na: 1 --><!-- b:\n2 -->\n<!-- c: 3\n-->"
        headers = list(bakesite.read_headers(text))
        self.assertEqual(headers, [("a", "1", 13), ("b", "2", 27), ("c", "3", 40)])

    def test_no_header(self):
        headers = list(bakesite.read_headers("Foo"))
        self.assertEqual(headers, [])

    def test_empty_string(self):
        headers = list(bakesite.read_headers(""))
        self.assertEqual(headers, [])
